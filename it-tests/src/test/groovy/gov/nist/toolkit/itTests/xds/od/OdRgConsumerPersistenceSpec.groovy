 ...
 */
class OdRgConsumerPersistenceSpec extends ToolkitSpecification {
    ...
    def setupSpec() {   // one time setup done when class launched
        ...
        repSites.add(rgConfig.getFullId())
    ...
    def 'Retrieve from the ODDS with Persistence Option'() {
        ...
        for (StepResult sr : results.get(0).stepResults) {
            for (Document d : sr.documents) {
                ...
                if (newRepository)
                    newRepository = (d.newRepositoryUniqueId!=null)
                if (!duplicates)
        ...
        persistedRetrieveRs.add(api.runTest(testSession, rgConfig.getFullId(), testId, sections, params, stopOnFirstError).get(0))
