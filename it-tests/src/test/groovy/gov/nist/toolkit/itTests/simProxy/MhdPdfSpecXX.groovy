...
class MhdPdfSpecXX extends ToolkitSpecification {
    ...
    @Shared String envName = 'default' // ximproxy runs everything in the default envrionment
    ...
    def 'submit and read binary'() {
        when: 'submit pdb with known entryUUID urn:uuid:1e404af3-077f-4bee-b7a6-a9be97e1ce34'
        def params = [ :]
        TestInstance testInstance = new TestInstance('pdb_single_doc_recresp_pdf') // Note: bill__mhd site name is hard-coded in singledocsubmit.xml
...
//        }
        ...
        when:
        List<Result> results3 = api.runTest(testSession, mhdName, testInstance, ['read_document_reference'], params, true)

        then:
        results3.size() == 1
        results3.get(0).passed()



        when:  'read binary'
        List<Result> results2 = api.runTest(testSession, mhdName, testInstance, ['read_document_binary'], params, true)
