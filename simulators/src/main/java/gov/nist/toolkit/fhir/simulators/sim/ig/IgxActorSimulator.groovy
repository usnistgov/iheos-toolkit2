...
public class IgxActorSimulator extends GatewaySimulatorCommon {
   ...
   public IgxActorSimulator(DsSimCommon dsSimCommon, SimulatorConfig simulatorConfig) {
      super(dsSimCommon.simCommon, dsSimCommon);
      this.db = dsSimCommon.simCommon.db;
      setSimulatorConfig(simulatorConfig);
   }

   public IgxActorSimulator() {}
   ...
   public boolean run(TransactionType transactionType, MessageValidatorEngine mvc, String validationPattern) throws IOException {
      ...
      logger.info("IgxActorSimulator: run - transactionType is " + transactionType);
      ...
      switch (transactionType) {
         ...
         // extract retrieve request
            AbstractMessageValidator mv = dsSimCommon.getMessageValidatorIfAvailable(SoapMessageValidator.class);
            if (mv == null || !(mv instanceof SoapMessageValidator)) {
               er.err(Code.XDSRegistryError, "IGX Internal Error - cannot find SoapMessageValidator instance", "InitiatingGatewayXActorSimulator", "");
               dsSimCommon.sendErrorsInRegistryResponse(er);
               return false;
            }
            SoapMessageValidator smv = (SoapMessageValidator) mv;
         ...
         default:
            er.err(Code.XDSRegistryError, "Don't understand transaction " + transactionType, "InitiatingGatewayXActorSimulator", "");
