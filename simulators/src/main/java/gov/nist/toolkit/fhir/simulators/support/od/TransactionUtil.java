package gov.nist.toolkit.fhir.simulators.support.od;
...
public class TransactionUtil {
    ...
    static public Result register(Session session, TestSession testSession, TestInstance testInstance, SiteSpec registry, Map<String, String> params, List<String> sections) {
        ...
        try {
            ...
            results = XdsTestServiceManager.runTestplan(session.getCurrentEnvironment(), testSession, registry, testInstance, sections, params, stopOnFirstError, xdsTestServiceManager, true);
...
//            TestLogDetails ts = xdsTestServiceManager.getTestDetails(testInstance, section);
            ...
            if (persistenceOption) {
                ...
                if (ded.getSnapshot() == null /* first retrieve attempt */ || ALL_OD_DOCS_SUPPLIED != ded.getSnapshot().getSupplyStateIndex() /* subsequent attempt until the last */ ) {
                    results = XdsTestServiceManager.runTestplan(session.getCurrentEnvName(), testSession, repository, testInstance, sections, params, stopOnFirstError, xdsTestServiceManager, true);
