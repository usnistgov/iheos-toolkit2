...
@TypeChecked
public class RGXActorSimulator extends GatewaySimulatorCommon implements MetadataGeneratingSim {
   SimDb db;
   static Logger logger = Logger.getLogger(RegistryActorSimulator.class.getName());
   Metadata m;
   MessageValidatorEngine mvc;

   public RGXActorSimulator(SimCommon common, DsSimCommon dsSimCommon, SimDb db, SimulatorConfig simulatorConfig) {
      super(common, dsSimCommon);
      this.db = db;
      setSimulatorConfig(simulatorConfig);
   }

   public RGXActorSimulator(DsSimCommon dsSimCommon, SimulatorConfig simulatorConfig) {
      super(dsSimCommon.simCommon, dsSimCommon);
      this.db = dsSimCommon.simCommon.db;
      setSimulatorConfig(simulatorConfig);
   }

   public RGXActorSimulator() {}
      ...
      switch (transactionType) {
         case TransactionType.XC_RETRIEVE:
            ...
            if (mv == null || !(mv instanceof SoapMessageValidator)) {
               er.err(Code.XDSRegistryError, "RGX Internal Error - cannot find SoapMessageValidator instance", "RespondingGatewayActorSimulator", "");
               returnRetrieveError();
               return false;
            }
            ...
            if (asce == null) {
               er.err(Code.XDSUnknownCommunity, "RGX Internal Error - homeCommunityId not configured", this, "");
               returnRetrieveError();
               return false;
            }
            ...
            // base configuration of the RGX or it could be another Repository sim linked as being part of the AD.
         ...
         case TransactionType.XC_QUERY:
            ...
            if (asce == null) {
               er.err(Code.XDSRegistryError, "RGX Internal Error - homeCommunityId not configured", this, "");
               dsSimCommon.sendErrorsInAdhocQueryResponse(er);
               return false;
            }
         ...
         for (OMElement obj : metadata.getExtrinsicObjects()) {
            ...
            if (submittedHome == null || submittedHome.equals("")) {
               er.err(Code.XDSMissingHomeCommunityId, "Update does not contain a homeCommunityId", "RGXActorSimulator", "");
               dsSimCommon.sendErrorsInAdhocQueryResponse(er);
               return false;
            }
            if (!submittedHome.equals(configuredHome)) {
               er.err(Code.XDSMissingHomeCommunityId, "Update has updated homeCommunityId - it no longer matches RespondingGateway configuration (" + configuredHome + ")", "RGXActorSimulator", "");
               dsSimCommon.sendErrorsInAdhocQueryResponse(er);
               return false;
            }
         }
         ...
         default:
            ...
            dsSimCommon.sendFault("RGXActorSimulator: Don't understand transaction " + transactionType, null);
            return true;
      }
