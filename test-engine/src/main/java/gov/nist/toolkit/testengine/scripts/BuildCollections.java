...
public class BuildCollections extends HttpServlet {
   ...
   File testkitOut;
   /**
    * directories within a test kit which may contain tests.
    */
   ...
   void write() throws IOException {
      ...
      // This layer is necessary because multiple collectionNames can map to
      // single
      // actorCollection since some collections are based on the actor and some
      // on transactions
   ...
   void add(String collection, String testnum) {
      ...
      if (!c.contains(testnum)) c.add(testnum);
   }
