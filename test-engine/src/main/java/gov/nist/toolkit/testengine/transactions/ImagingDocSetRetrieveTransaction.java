...
//				compileUseRepositoryUniqueId(m, use_repository_unique_id);
			...
			if (is_xca) {
				...
				if (repositoryUniqueId == null || repositoryUniqueId.equals("")) {
					...
					"/*[local-name()='SeriesRequest']" +
					"/*[local-name()='DocumentRequest']" +
					"/*[local-name()='RepositoryUniqueId']/text()";
					try {
						...
						fatal("ImagingDocSetRetrieveTransaction: run(): XPATH error extracting repositoryUniqueId");
					}
				...
				if (useReportManager == null &&
					...
					fatal("ImagingDocSetRetrieveTransaction: no repositoryUniqueId");
				}

				// assign endpoint
				parseIDSEndpoint(repositoryUniqueId, testConfig.secure);
			...
			try {
				...
				rad69.setUseIG(useIG);
				rad69.setExpectedMimeType(this.expected_mime_type);
...
//				testLog.add_name_value(instruction_output, "Result", result);
	...
	private HashMap<String, RetrievedDocumentModel> build_request_info(OMElement metadata_ele) throws XdsException {
		...
		for (OMElement document_request : XmlUtil.childrenWithLocalName(metadata_ele, "DocumentRequest")) {
			...
			//			if (eo == null)
			//			throw new XdsInternalException("ImagingDocSetRetrieveTransaction: build_request_info: reference document " + doc_uid + " not available");
