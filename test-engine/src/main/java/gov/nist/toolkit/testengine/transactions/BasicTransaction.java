...
public abstract class BasicTransaction {
   ...
   HashMap <String, String> local_linkage_data = new HashMap <>(); // dangerous,
                                                                   // most of
                                                                   // linkage
                                                                   // kept
                                                                   // inside
   ...
   boolean isSQ;
   boolean noMetadataProcessing = false; // example Retrieve request - no
                                         // metadata to process
   ...
   protected abstract String getBasicTransactionName(); // trans name without
                                                        // possible .as suffix
   ...
   void applyTransactionSettings() {
      if (transactionSettings == null) return;
      if (transactionSettings.assignPatientId != null) assign_patient_id = transactionSettings.assignPatientId;
      async = false;
      if (transactionSettings.siteSpec != null) // null for Direct
         async = transactionSettings.siteSpec.isAsync;
      if (transactionSettings.patientId != null) planContext.setPatientId(transactionSettings.patientId);
      if (transactionSettings.altPatientId != null) planContext.setAltPatientId(transactionSettings.altPatientId);
   }
   ...
   protected StepContext getStep() {
      return s_ctx;
   }

   protected PlanContext getPlan() {
      return planContext;
   }
   ...
   public void doRun() throws Exception {
      ...
      if (trans == null) fatal("Internal error: No transaction name declared");

      if (async) xds_version = BasicTransaction.xds_b;

      if (trans.equals("sq") || trans.equals("pr") || trans.equals("r")) {
         // if (async)
         // trans = trans + ".as";
         // else
         if (isB()) trans = trans + ".b";
         else trans = trans + ".a";
      }
      // else if (async)
      // trans = trans + ".as";
      ...
      if (defaultEndpointProcessing && ttype != null) parseEndpoint(ttype);

      Metadata metadata = prepareMetadata();
      if (metadata != null) request_element = metadata.getRoot();

      // reportManagerPreRun(request_element); // must run before
      // prepareMetadata (assign uuids)
   ...
   protected void reportManagerPreRun(OMElement metadata_element) throws XdsInternalException, XdsInternalException {
      ...
      if (useReportManager != null) {
         ...
         try {
            sectionLogs.put("THIS", new LogFileContent(getPlan().getLog(),
               true /* incomplete is ok */));
         ...
         try {
            ...
            // because useReportManager.resolve below will take care of many
            // problems
            // fatal("UseReportManager failed to load necessary prior-test log
            // files", e);
            // return;
         }
   ...
   public String toString() {
      ...
      return new StringBuffer()
         ...
         .append(xdsVersionName()).append("\n").append("use_id = ").append((use_id == null) ? null : use_id.toString())
         .append("use_xpath = ").append((use_xpath == null) ? null : use_xpath.toString()).append("use_object_ref = ")
         .append((use_object_ref == null) ? null : use_object_ref.toString()).append("use_repository_unique_id = ")
         .append((use_repository_unique_id == null) ? null : use_repository_unique_id.toString()).append("data_refs = ")
         .append((data_refs == null) ? null : data_refs.toString()).append("assertions = ")
         .append((assertions == null) ? null : assertions.toString()).append("endpoint = ").append(endpoint)
   ...
   public String xdsVersionName() {
      if (xds_version == BasicTransaction.xds_none) return "None";
      if (xds_version == BasicTransaction.xds_a) return "XDS.a";
      if (xds_version == BasicTransaction.xds_b) return "XDS.b";
      return "Unknown";
   }
   ...
   String xds_version_name() {
      if (xds_version == xds_a) return "XDS.a";
      if (xds_version == xds_b) return "XDS.b";
      return "Unknown";
   }
   ...
   void validate_registry_response_in_soap(OMElement env, int metadata_type)
      throws XdsInternalException, MetadataValidationException, MetadataException {
      ...
      if (hdr == null) throw new XdsInternalException("Expected 'Header' but found nothing instead");
      ...
      if (!(next instanceof OMElement)) throw new XdsInternalException(
         "Body not of type OMElement, instead found " + ((next == null) ? "null" : next.getClass().getName()));
      OMElement body = (OMElement) next;
      if (body == null) throw new XdsInternalException("Expected 'Body' but found nothing instead");
   ...
   void validate_registry_response(OMElement result, int metadata_type)
      throws XdsInternalException, MetadataValidationException, MetadataException {
   ...
   void validate_registry_response_no_set_status(OMElement registry_result, int metadata_type)
      throws XdsInternalException, MetadataValidationException, MetadataException {
      ...
      if (!"Fault".equals(status)) validateSchema(registry_result, metadata_type);
      ...
      try {
         ...
         rel = new RegistryErrorListGenerator(
            (xds_version == xds_a ? RegistryErrorListGenerator.version_2 : RegistryErrorListGenerator.version_3));
      }

      ArrayList <String> validatorErrors =
         new RegistryResponseParser(rel.getRegistryErrorList()).get_error_code_contexts();
      ...
      if (expectedErrorCode != null && !expectedErrorCode.equals("")) {
         ...
         if (!errorCodes.contains(expectedErrorCode)) {
            s_ctx.set_error("Expected errorCode of " + expectedErrorCode
               + "\nDid getRetrievedDocumentsModel errorCodes of " + errorCodes);
            step_failure = true;
         }
      }
   }
   ...
   protected void validateSchema(OMElement registry_result, int metadata_type) throws XdsInternalException {
   ...
   void eval_expected_status(String status, ArrayList <String> returned_code_contexts) throws XdsInternalException {
      ...
      if (xds_version == xds_a && isSQ == false) {
         ...
         if (!currentStatus.isNamespaceOk()) {
            s_ctx.set_error("Status is " + status + " , expected status is "
               + MetadataSupport.response_status_type_namespace + getExpectedStatusString());
            step_failure = true;
         } else {
      ...
      // if expected error message specified then expected status must be
      // Failure
      ...
      if (!s_ctx.getExpectedErrorMessage().equals("")) {
         ...
         if (!error_message_found) {
            s_ctx
               .set_error("Did not find expected string in error messages: " + s_ctx.getExpectedErrorMessage() + "\n");
            step_failure = true;
         }
      }
   }
   ...
   void add_step_status_to_output() {
      ...
      if (step_failure) testLog.add_name_value(instruction_output, "StepStatus", "Failure");
      else testLog.add_name_value(instruction_output, "StepStatus", "Success");
   }
   ...
   protected void compileUseRepositoryUniqueId(Metadata m, ArrayList <OMElement> useInputMetadata)
      throws XdsInternalException, MetadataException, FactoryConfigurationError {
   ...
   protected void compileUseIdLinkage(Metadata m, ArrayList use_id)
      throws XdsInternalException, MetadataException, FactoryConfigurationError {
   ...
   protected void compileUseObjectRefLinkage(Metadata m, ArrayList use_object_ref)
      throws XdsInternalException, MetadataException, FactoryConfigurationError {
      Linkage l = new Linkage(testConfig, instruction_output, m);
   ...
   protected void compileUseXPathLinkage(Metadata m, ArrayList use_xpath)
      throws XdsInternalException, MetadataException, FactoryConfigurationError {
      Linkage l = new Linkage(testConfig, instruction_output, m);
      ...
      try {
         ...
         throw new XdsInternalException(
            s_ctx.get("step_id") + ": " + e.getMessage() + ExceptionUtil.exception_details(e));
      }

   }

   private void parseEndpoint(TransactionType trans) throws Exception {
      endpoint = this.s_ctx.getRegistryEndpoint(); // this is busted, always
                                                   // returns null
      if (endpoint == null || endpoint.equals("") || testConfig.endpointOverride) { // boolean
                                                                                    // async
                                                                                    // =
                                                                                    // false;
         if (testConfig.verbose) System.out.println("endpoint coming from actors.xml");
         ...
         if (trans.usesTraditionalTransactions()) {
            ...
            if (endpoint == null || endpoint.equals("")) fatal("No endpoint specified for transaction " + trans
               + " and XDS version " + xds_version_name() + " and secure = " + testConfig.secure + " on site "
               + testConfig.site.getSiteName() + "\nactor config is " + testConfig.site.toString());
            testLog.add_name_value(instruction_output, "Endpoint", endpoint);
         }
      } else {
         if (testConfig.verbose) System.out.println("endpoint coming from testplan.xml");
      }
   ...
   protected void log_metadata(OMElement submission) throws XdsInternalException {
      testLog.add_name_value(instruction_output, "InputMetadata", Util.deep_copy(submission));
   }

   Metadata prepareMetadata()
      throws XdsInternalException, XdsInternalException, MetadataException, FactoryConfigurationError {
      Metadata metadata = null;
      ...
      if (parse_metadata) {
         ...
         if (assign_patient_id) {
            // System.out.println("=============================
            // assign_patient_id in
            // BasicTransaction#prepareMetadata()==============================");
         ...
         if (assign_uids) {
            ...
            if (reportManager == null) reportManager = new ReportManager(testConfig);
         ...
         if ("true".equals(Configuration.getProperty("testclient.addTestAsAuthor"))) {
            if (metadata != null) metadata.addAuthorPersonToAll(id);
         }
      ...
      if (use_id.size() > 0) compileUseIdLinkage(metadata, use_id);
      if (use_object_ref.size() > 0) compileUseObjectRefLinkage(metadata, use_object_ref);
      if (use_xpath.size() > 0) compileUseXPathLinkage(metadata, use_xpath);
   ...
   // protected void logInputMetadata(OMElement metadata) throws
   // XdsInternalException {
   // s_ctx.add_name_value(instruction_output, "InputMetadata",
   // Util.deep_copy(metadata));
   // }
   ...
   protected void parseBasicInstruction(OMElement part) throws XdsInternalException {
      ...
      switch (part_name) {
         ...
         case "Report":
            ...
            break;
         case "UseReport":
            if (useReportManager == null) useReportManager = new UseReportManager(testConfig);
            useReportManager.add(part);
            break;
         case "ParseMetadata":
            String value = part.getText();
            testLog.add_name_value(this.instruction_output, "ParseMetadata", value);
            if (value.equals("False")) parse_metadata = false;
            break;
         case "NoMetadata":
            noMetadataProcessing = true;
            break;
         case "SOAPHeader":
            if (additionalHeaders == null) additionalHeaders = new ArrayList <OMElement>();
         ...
         case "WSSECHeader":
            ...
            /*
             * if(transactionSettings.issaml){ if (wsSecHeaders == null)
             * wsSecHeaders = new ArrayList<OMElement>(); OMElement omElement =
             * WSSESecurityHeaderUtil.getWSSecOMElement();
             * System.out.println(omElement.getText());
             * System.out.println("***************");
             * System.out.println(omElement.toString());
             * //this.s_ctx.add_name_value(omElement, "WSSECHeader",
             * part.getFirstElement());
             * this.s_ctx.add_name_value(this.instruction_output, "WSSECHeader",
             * omElement);
             * 
         ...
         case "WaitBefore":
            ...
            if (milliseconds == 0) fatal("WaitBefore: zero delay requested");
            try {
               long t0, t1, diff;
   ...
   protected DocDetails getDocDetailsFromLogfile(OMElement uri_ref) throws FactoryConfigurationError,
      XdsInternalException, XdsInternalException, MetadataException, MetadataValidationException {
      ...
      if (m.getExtrinsicObjectIds().size() == 0) {
         fail("No ExtrinsicObjects found in log file " + log_file + " step " + step_id + " with transaction type "
            + transaction_type);
   ...
   public String getEndpointService() throws XdsInternalException {
      if (endpoint == null || endpoint.equals("")) throw new XdsInternalException("Endpoint not initialized");
      int doubleSlash = endpoint.indexOf("//");
      if (doubleSlash == -1) throw new XdsInternalException("Endpoint contains no double slash");
      int singleSlash = endpoint.indexOf("/", doubleSlash + 2);
      if (singleSlash == -1) throw new XdsInternalException("Endpoint has no service");
   ...
   protected void soapCall(OMElement requestBody) throws Exception {
      ...
      if (testConfig.saml) {
         System.out.println("\tAxis2 client Repository: " + testConfig.testmgmt_dir + File.separator + "rampart"
            + File.separator + "client_repositories");
         System.out.println("\tEnabling WSSEC ...");
         soap.setRepositoryLocation(
            testConfig.testmgmt_dir + File.separator + "rampart" + File.separator + "client_repositories");
      }
      ...
      /*
       * if (wsSecHeaders != null) { for (OMElement hdr : wsSecHeaders) try {
       * soap.addSecHeader(Util.deep_copy(hdr)); } catch (XdsInternalException
       * e) { s_ctx.set_error(e.getMessage()); failed(); logSoapRequest(soap); }
       * }
       */
      ...
      try {
         ...
         soap.soapCall(requestBody, endpoint, useMtom, // mtom
            useAddressing, // WS-Addressing
            soap_1_2, // SOAP 1.2
            getRequestAction(), getResponseAction(), this.planContext.getExtraLinkage());
         ...
         try {
            if (!s_ctx.expectFault()) s_ctx.set_fault(e);
   ...
   // s_ctx.add_name_value(instruction_output, "OutHeader",
   // soap.getOutHeader());
   // } catch (Exception e) {
