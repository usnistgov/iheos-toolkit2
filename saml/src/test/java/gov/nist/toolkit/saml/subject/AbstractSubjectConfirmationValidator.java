...
import gov.nist.toolkit.saml.exception.ValidationException;
public abstract class AbstractSubjectConfirmationValidator {
	/**
	   ...
	   // private Logger log = LoggerFactory.getLogger(AbstractSubjectConfirmationValidator.class);
		...
		/**
		 * Helper method that trims a string and returns null if blank.
		 * Replaces DatatypeHelper.safeTrimOrNullString()
		 *
		 * @param input the string to trim
		 * @return trimmed string or null if blank
		 */
		private String safeTrimOrNullString(String input) {
	    ...
	    protected ValidationResult validateNotBefore(SubjectConfirmation confirmation, Assertion assertion,
	        ...
	        Instant skewedNow = Instant.now().plusMillis((int) getClockSkew(context));
	    ...
	    */
	    ...
	    protected ValidationResult validateNotOnOrAfter(SubjectConfirmation confirmation, Assertion assertion,
	            ValidationContext context) {
	        Instant skewedNow = Instant.now().minusMillis((int) getClockSkew(context));
	    ...
	    protected ValidationResult validateRecipient(SubjectConfirmation confirmation, Assertion assertion,
	            ValidationContext context) {
			String recipient = safeTrimOrNullString(confirmation.getSubjectConfirmationData().getRecipient());
	    ...
	    protected ValidationResult validateAddress(SubjectConfirmation confirmation, Assertion assertion,
			...
			String address = safeTrimOrNullString(confirmation.getSubjectConfirmationData().getAddress());
