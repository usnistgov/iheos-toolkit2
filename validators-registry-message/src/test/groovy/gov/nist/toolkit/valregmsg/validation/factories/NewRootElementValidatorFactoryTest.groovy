 ...
 *
 */
class NewRootElementValidatorFactoryTest extends Specification {
    def pnrXmlString
    OMElement xml
    ...
    def 'Pnr validation based on validation context'() {
        ...
        mvc = NewValidationContextValidationFactory.validateBasedOnValidationContext(
                erb, xml, mvc, vc, rvi
        )
        mvc.run()
    ...
    def 'Pnr validation based on root element'() {
        ...
        when: 'Run validations'
        mvc = NewRootElementValidatorFactory.validateBasedOnRootElement(
