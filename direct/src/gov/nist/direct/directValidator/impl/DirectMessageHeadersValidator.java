...
public class DirectMessageHeadersValidator implements MessageHeadersValidator {
	...
	public void validateReturnPath(ErrorRecorder er, String returnPath, boolean wrapped) {
		...
		if(returnPath.equals("")) {
			er.warning("103-105", "Return Path", "Not present", "Should be present (\"<\" [ A-d-l \":\" ] Mailbox \">\")", "RFC 5321: Section 4.4 - RFC 5322: Section 3.6.7");
			return;
		}
		
		if(ValidationUtils.validateAddrSpec(returnPath)) {
			er.success("103-105", "Return Path", txtReturnPath, "\"<\" [ A-d-l \":\" ] Mailbox \">\"", rfc);
		} else {
			er.error("103-105", "Return Path", txtReturnPath, "\"<\" [ A-d-l \":\" ] Mailbox \">\"", rfc);
		}
