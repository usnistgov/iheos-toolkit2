MHD Document Recipient/Responder Lifecyle (PDF)
...
<p>Please note that currently read_document_binary and read_document_contents sections fail against the internal
Toolkit simulators with an incorrect byte count returned (one less than is sent).  This has only been detected with PDFs
so far but other non-text formats are yet to be tested.
