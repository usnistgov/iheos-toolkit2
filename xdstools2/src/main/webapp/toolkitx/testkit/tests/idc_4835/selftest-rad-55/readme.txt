<h3>Self-test: Send WADO Retrieve (RAD-55) Request to IDS sim.</h3>

<p/>When the test section is run:
<ol>
<li/>The Image Document Consumer simulator (IDC sim) sends a WADO Retrieve 
(RAD-55) to the Imaging Document Source System Under Test (IDS SUT).
<li/>The IDS SUT is expected to process the test and generate a response
containing the requested image.
<li/>The IDC sim will process the RAD-55 response and store the received
image.
</ol>