Partial Success, Single GW

<h2>Partial Success, Single Responding Gateway</h2>

<h3>Purpose / Configuration</h3>
<p>A retrieve request is sent to the  Initiating Imaging Gateway for two
imaging studies from Community A using one transfer syntax value.</p>
<table border="1">
...
(IDS A1 sim).
<li/>The IDS A1 sim will process the RAD-69 request, but will
return images from only one study.
<li/>The RIG A sim will process the RAD-69 response, generating a RAD-75
response, which will be returned to the IIG SUT.
The RAD-75 response generated by RIG A simulator will contain
a partial success status. It will contain images from one imaging
study but not both.
