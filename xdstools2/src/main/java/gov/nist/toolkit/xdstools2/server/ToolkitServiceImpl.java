...
//                + ":" + props.getToolkitPort()  + servletContext().getServletContextName() +"Xdstools2.html");
    ...
    @Override
    public RawResponse buildIdcxcaTestOrchestration(BuildIdcxcaTestOrchestrationRequest request) {
        Session s = getSession();
        if (s == null) return RawResponseBuilder.build(new NoServletSessionException(""));
        return new OrchestrationManager().buildIdcxcaTestEnvironment(s, request.getIdcxcaOrchestrationRequest());
    }
    @Override
