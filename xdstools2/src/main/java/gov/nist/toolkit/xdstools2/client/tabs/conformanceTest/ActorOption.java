...
public class ActorOption {
    ...
    void loadTests(final AsyncCallback<List<TestInstance>> callback) {
        ...
        if (optionId == null || optionId.equals("")) {
            request = new GetCollectionRequest(XdsTools2Presenter.data().getCommandContext(), "actorcollections", actorTypeId);
        } else {
            request = new GetCollectionRequest(XdsTools2Presenter.data().getCommandContext(), "collections", actorTypeId + "_" + optionId);
        }
