...
public class ConformanceTestTab extends ToolWindow implements TestRunner, SiteManager, TestsHeaderView.Controller {
   ...
   private final FlowPanel toolPanel = new FlowPanel(); // Outer-most panel for
                                                        // the tool
   private final FlowPanel initializationPanel = new FlowPanel();
   private final FlowPanel testsPanel = new FlowPanel(); // panel for displaying
                                                         // tests
   private final TabBar tabBar = new TabBar(); // tab bar at the top for
                                               // selecting actor types
   ...
   private AbstractOrchestrationResponse orchestrationResponse; // can be any of
                                                                // the following
                                                                // - contains
                                                                // common
                                                                // elements
   ...
   /**
    * currentActorTypeDescription is initialized late so calling this when it is
    * available updates the display since it could not be constructed correctly
    * at first. This must be called after testCollectionDefinitionDAOs is
    * initialized.
    */
   ...
   /*
    * teal box on right end of tab bar with current test context data
    */
   private void displayTestSessionDisplay() {
      testSessionDescription.setHTML("Test Context<br />" + "Environment: " + getEnvironmentSelection() + "<br />"
         + "TestSesson: " + getCurrentTestSession() + "<br />" + "SUT: " + getSiteName());
   }
   ...
   private String getSelectedTestCollection() {
      ...
      if (selected < testCollectionDefinitionDAOs
         .size()) { return testCollectionDefinitionDAOs.get(selected).getCollectionID(); }
      return null;
   }
   ...
   private int getTestCollectionIndex(String name) {
      ...
      for (TestCollectionDefinitionDAO def : testCollectionDefinitionDAOs) {
         if (def.getCollectionID().equals(name)) return i;
         i++ ;
      }
      return -1;
   }
   ...
   private void loadTestCollections() {
      ...
      getToolkitServices().getTestCollections("actorcollections",
         new AsyncCallback <List <TestCollectionDefinitionDAO>>() {
            @Override
            public void onFailure(Throwable throwable) {
               new PopupMessage("getTestCollections: " + throwable.getMessage());
            }
            ...
            public void onSuccess(List <TestCollectionDefinitionDAO> testCollectionDefinitionDAOs) {
               ...
               // This is a little wierd being here. This depends on
               // initTestSession
               // which is set AFTER onTabLoad is run so run here - later in the
               // initialization
   ...
   private void displayTestCollection() {
      ...
      getToolkitServices().getCollectionMembers("actorcollections", currentActorTypeId,
         new AsyncCallback <List <String>>() {
            ...
            public void onSuccess(List <String> testIds) {
               ...
               for (String testId : testIds)
                  testInstances.add(new TestInstance(testId));
   ...
   private void displayTests(List <TestInstance> testInstances) {
      ...
      getToolkitServices().getTestsOverview(getCurrentTestSession(), testInstances,
         new AsyncCallback <List <TestOverviewDTO>>() {
            ...
            public void onSuccess(List <TestOverviewDTO> testOverviews) {
               ...
               getToolkitServices().getAutoInitConformanceTesting(new AsyncCallback <Boolean>() {
                  ...
                  public void onSuccess(Boolean aBoolean) {
                     if (aBoolean) orchInit.handleClick(null); // auto init
                                                               // orchestration
                  }
               });
   ...
   private void orchestrationInitialization() {
      if (isRepSut()) {
         ...
         orchInit = new BuildIDSTestOrchestrationButton(this, initializationPanel, "Initialize Test Environment");
         initializationPanel.add(orchInit.panel());
   ...
   private boolean testDisplayExists(String name) {
      return testDisplays.containsKey(name);
   }
   ...
   private void displayTest(TestOverviewDTO testOverview) {
      ...
      if (testOverview.isRun()) {
         Image status = (testOverview.isPass()) ? new Image("icons2/correct-24.png")
      ...
      if (!isNew) updateTestsOverviewHeader();
   ...
   /**
    * Handles click on a test run button.
    */
   ...
   private void displaySections(TestOverviewDTO testOverview, FlowPanel parent) {
      ...
      for (String sectionName : testOverview.getSectionNames()) {
         ...
         parent.add(new TestSectionComponent(getCurrentTestSession(), new TestInstance(testOverview.getName()),
            sectionOverview, this).asWidget());
      }
   }

   /* if testInstance contains a sectionName then run that section, otherwise
    * run entire test.
    */
   @Override
   public void runTest(final TestInstance testInstance, final TestDone testDone) {
      ...
      if (ActorType.REPOSITORY.getShortName().equals(currentActorTypeId) && repOrchestrationResponse != null) {
         ...
         // parms.put("$patientid$",
         // regOrchestrationResponse.getPid().asString());
      } else { // mostly for early debugging
         parms.put("$patientid$", "P20160907182617.2^^^&1.3.6.1.4.1.21367.2005.13.20.1000&ISO");
      }

      try {
         getToolkitServices().runTest(getEnvironmentSelection(), getCurrentTestSession(), getSitetoIssueTestAgainst(),
            testInstance, parms, true, new AsyncCallback <TestOverviewDTO>() {
               @Override
               public void onFailure(Throwable throwable) {
                  new PopupMessage(throwable.getMessage());
               }
               ...
               public void onSuccess(TestOverviewDTO testOverviewDTO) {
                  ...
                  if (testDone != null) testDone.onDone(testInstance);
               }
            });
      } catch (Exception e) {
