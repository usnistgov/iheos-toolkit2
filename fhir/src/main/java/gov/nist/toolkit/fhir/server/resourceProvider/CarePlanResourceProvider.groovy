...
class CarePlanResourceProvider implements IResourceProvider{
    ...
    public MethodOutcome createCarePlan(@ResourceParam CarePlan theCarePlan,
        ...
        validateResource(theCarePlan);

        return new ToolkitResourceProvider(getResourceType(), requestDetails).createOperation(theCarePlan)
    }

    @Read()
    public CarePlan getResourceById(@IdParam IdType theId, RequestDetails requestDetails) {
    ...
    public List<CarePlan> getCarePlan(
            @RequiredParam(name = CarePlan.SP_SUBJECT) StringParam theSubject,
            RequestDetails requestDetails) {
        ToolkitResourceProvider tk = new ToolkitResourceProvider(getResourceType(), requestDetails)
        ...
        term = new Term(CarePlan.SP_SUBJECT, theSubject.value)
    ...
    def validateResource(CarePlan theLocation) {}
}
