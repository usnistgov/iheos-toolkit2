...
public class Soap implements SoapInterface {
	...
	String repositoryLocation   = null; // this is axis2 repository - used only with useSaml / Seems used to store axis modules..
	SecurityParams securityParams; //contextual security info used by SAML/TLS to access the keystore 
	 ...
	 */
	public OMElement getInHeader() { return inHeader; }
	/* (non-Javadoc)
	 * @see gov.nist.registry.common2.axis2soap.SoapInterfac#getOutHeader()
	 */
	public OMElement getOutHeader() { return outHeader; }

	public Soap() {
	}

	/* (non-Javadoc)
	 * @see gov.nist.registry.common2.axis2soap.SoapInterfac#addHeader(org.apache.axiom.om.OMElement)
	 */
	...
	/* (non-Javadoc)
	 * @see gov.nist.registry.common2.axis2soap.SoapInterfac#addSecHeader(org.apache.axiom.om.OMElement)
	 */
	...
	ConfigurationContext buildConfigurationContext() throws XdsInternalException, AxisFault {
		...
		if (!rl.exists() || !rl.isDirectory())
			throw new XdsInternalException("Axis2 repository location, " + repositoryLocation + ", does not exist or is not a directory");
/*
		File ax = new File(repositoryLocation + File.separator + "conf" + File.separator + "axis2.xml");
		if (!ax.exists()) 
			throw new XdsInternalException("Configuration file, " + ax + " does not exist");
*/
		...
		try {
			...
			//TODO REMOVE ?? exact same call = exact same result. I am puzzled -Antoine 
			cc = ConfigurationContextFactory.createConfigurationContextFromFileSystem(repositoryLocation); 
	...
	SOAPEnvelope createSOAPEnvelope() throws LoadKeystoreException {
		...
		if (useWSSEC) {
			//OMNamespace ns = OMAbstractFactory.getOMFactory().createOMNamespace("http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd", "wsse");
			//securityHeader = OMAbstractFactory.getOMFactory().createOMElement("Security", ns);
	...
	//		} 
	//		catch (Exception e) {
	//			throw new XdsException("Could not parse additionalHeader: error was: " + e.getMessage() + "\n\tHeader was: " + hdrStr );
	//		} 
	//	}
	//}
	 ...
	 */
	public void soapCallWithWSSEC() 
	throws  XdsInternalException, AxisFault, EnvironmentNotSelectedException, LoadKeystoreException {
		...
		//TODO CHECK engaging the addressing module. Should it really be engaged a each soap call? -Antoine
...
//				System.out.println("Bill says Axis Fault was " + e.getMessage());
		...
		if (useWSSEC)
		{
			...
			//TODO CHECK - This should not be necessary. This operation is already created by the serviceClient constructor! -Antoine
			...
			//vbeera: The below 2 lines is the 2nd fix/solution which is a potential one. Comment the above fix and unComment the below 2 lines in order to use 2nd fix.
			//AxisConfiguration ac = cc.getAxisConfiguration();
			//((Phase)ac.getInFlowPhases().get(0)).addHandler(new PatchForMustUnderstand.SecurityHandler());
		...
		setOptions(options);
		options.setProperty(AddressingConstants.ADD_MUST_UNDERSTAND_TO_ADDRESSING_HEADERS, Boolean.TRUE);//vbeera: modified		
		
		// This creates an HTTPClient using the requested keystore and truststore
...
//				Protocol.registerProtocol("https", authhttps);
		...
		new OMFormatter(result).toString();  // this forces full read before channel is closed
	...
	// JVM level controls on the certs used and specify certs on a per-connection basis.
	 ...
	 */
	public OMElement soapCall() 
	throws  LoadKeystoreException, XdsInternalException, AxisFault, XdsFormatException, EnvironmentNotSelectedException {		
	...
	throws XdsFormatException, XdsInternalException {
		...
		if ( mtom != responseMtom)
			if (mtom) {
				throw new XdsFormatException("Request was MTOM format but response was SIMPLE SOAP", WsDocRef.MTOM_in_MTOM_out);
			} else {
				throw new XdsFormatException("Request was SIMPLE SOAP but response was MTOM", WsDocRef.MTOM_in_MTOM_out);
			}


		// toolkit sometimes depends on Synapse ESB to translate async to sync for getting through firewalls
		// sometimes Synapse gets subborn about passing on the correct WS:action on return
		if (async)
	 ...
	 * TODO CHANGE this should be set once for the execution! 
	 * TODO CHANGE hardcoded parameters should be accessible in some easily identifiable place. What do they mean? -Antoine
	 */
	...
	throws AxisFault {
		...
		//TODO CHECK WS-Addressing Action / SOAP Action string - what does this really mean? -Antoine
...
//				System.out.println("engageModule (kinda) try " + i);
				try {
					lastFault = null;
					serviceClient.engageModule("addressing");
					done = true;
...
*/
	...
	void verify_returned_action(String expected_return_action, String alternate_return_action) throws XdsInternalException {
	...
	/* (non-Javadoc)
	 * @see gov.nist.registry.common2.axis2soap.SoapInterfac#soapCall(org.apache.axiom.om.OMElement, java.lang.String, boolean, boolean, boolean, java.lang.String, java.lang.String)
	 */
	...
	throws  XdsInternalException, AxisFault, XdsFormatException, EnvironmentNotSelectedException, LoadKeystoreException {
	...
	/* (non-Javadoc)
	 * @see gov.nist.registry.common2.axis2soap.SoapInterfac#getExpectedReturnAction()
	 */
	...
	/* (non-Javadoc)
	 * @see gov.nist.registry.common2.axis2soap.SoapInterfac#setExpectedReturnAction(java.lang.String)
	 */
	...
	/* (non-Javadoc)
	 * @see gov.nist.registry.common2.axis2soap.SoapInterfac#setAddressing(boolean)
	 */
