...
public class ProvideAndRetrieve extends CommonService {
	...
	public List<Result> run(SiteSpec site, String pid) {
		try {
			...
			TestId testId = new TestId("ProvideAndRetrieve");
...
//			Site si = session.siteServiceManager().getSites().getSite(session.siteSpec.name);
			...
			if (repuid == null) {
				Result r = ResultBuilder.RESULT(testId, null, new AssertionResult("Repository has no configured repositoryUniqueId","",false), null);
				return asList(r);
			}
			params.put("$repositoryUniqueId$", repuid);
			params.put("$repuid$", repuid);

			Result r = session.xdsTestServiceManager().xdstest(testId, sections, params, null, null, false);
			return asList(r);
		} catch (Exception e) {
